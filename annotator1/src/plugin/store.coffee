...
#     'My-Custom-Header': 'MyCustomValue'
#   })
#
...
class Annotator.Plugin.Store extends Annotator.Plugin
  ...
  options:
    ...
    # request header `HTTP-Method-Override` with the name of the desired
    # method.
    emulateHTTP: false
  ...
  _apiRequestOptions: (action, obj, onSuccess) ->
    ...
    if @options.emulateHTTP and method in ['PUT', 'DELETE']
      opts.headers = $.extend(opts.headers, {'HTTP-Method-Override': method})
      opts.type = 'POST'
